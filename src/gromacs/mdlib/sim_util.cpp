...
{
    //cout<<"void print_time is called!"<<endl;
    time_t finish;
    char   timebuf[STRLEN];
...
void print_date_and_time(FILE *fplog, int nodeid, const char *title,
                         double the_time)
{
    //cout<<"void print_date_and_time is called!"<<endl;
...
void print_start(FILE *fplog, t_commrec *cr,
                 ...
                 const char *name)
{
    //cout<<"void print_start is called!"<<endl;
    char buf[STRLEN];
...
#pragma omp parallel for num_threads(nt) schedule(static)
    //cout<<"In sim_util.cpp function sum_forces Before Add"<<endl;
    //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl;
    for (int i = 0; i < end; i++)
    {
        //cout<<"sim_util.cpp Before Add"<<endl;
        //cout<<"f["<<i<<"][0] "<<f[i][0]<<" f["<<i<<"][1] "<<f[i][1]<<" f["<<i<<"][2] "<<f[i][2]<<endl;
        //cout<<"forceToAdd["<<i<<"][0] "<<forceToAdd[i][0]<<" forceToAdd["<<i<<"][1] "<<forceToAdd[i][1]<<" forceToAdd["<<i<<"][2] "<<forceToAdd[i][2]<<endl;
	//cout<<"x["<<i<<"][0] "<<x[i][0]<<" x["<<i<<"][1] "<<x[i][1]<<" x["<<i<<"][2] "<<x[i][2]<<endl;
        rvec_inc(f[i], forceToAdd[i]);
        //cout<<"sim_util.cpp After Add"<<endl;
        //cout<<"f["<<i<<"][0] "<<f[i][0]<<" f["<<i<<"][1] "<<f[i][1]<<" f["<<i<<"][2] "<<f[i][2]<<endl;
        //cout<<"forceToAdd["<<i<<"][0] "<<forceToAdd[i][0]<<" forceToAdd["<<i<<"][1] "<<forceToAdd[i][1]<<" forceToAdd["<<i<<"][2] "<<forceToAdd[i][2]<<endl;
        //cout<<"x["<<i<<"][0] "<<x[i][0]<<" x["<<i<<"][1] "<<x[i][1]<<" x["<<i<<"][2] "<<x[i][2]<<endl;
        //cout<<fr->nbv->nbat->type[i]<<endl;
        //cout<<"f["<<i<<"][3] "<<f[i][3]<<endl;
        //cout<<"x["<<i<<"][3] "<<x[i][3]<<endl;
    }
    //cout<<"In sim_util.cpp function sum_forces After Add"<<endl;
    //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl;
...
static void pme_gpu_reduce_outputs(gmx_wallcycle_t            wcycle,
                                   ...
                                   real                       Vlr_q)
{
    //cout<<"static void pme_gpu_reduce_outputs is called!"<<endl;
...
{
    //cout<<"static void calc_virial is called!"<<endl;
    int    i;
...
{
    //cout<<"static void pull_potential_wrapper is called!"<<endl;
    t_pbc  pbc;
    real   dvdl;
...
static void pme_receive_force_ener(t_commrec       *cr,
                                   ...
                                   gmx_wallcycle_t  wcycle)
{
    //cout<<"static void pme_receive_force_ener is called!"<<endl;
...
{
    //cout<<"static void print_large_forces is called!"<<endl;
...
static void post_process_forces(t_commrec *cr,
                                ...
                                int flags)
{
    //cout<<"static void post_process_forces is called!"<<endl;
    //cout<<"sizeof(x) is: "<<sizeof(x)<<endl;
    //cout<<"sizeof(x[0]) is: "<<sizeof(x[0])<<endl;
    //cout<<"sizeof(f) is: "<<sizeof(f)<<endl;
    //cout<<"sizeof(f[0]) is: "<<sizeof(f[0])<<endl;
    if (fr->haveDirectVirialContributions)
    {
        ...
        if (vsite)
        {
             ...
             */
            //cout<<"spread visit f called"<<endl;
            //NOT called
        ...
        {
	    //cout<<"sum_forces(f, forceWithVirial->force_);"<<endl;
	    //This is Called; This is how sum_forces fxn is getting called
            /* Now add the forces, this is local */
            sum_forces(f, forceWithVirial->force_);
            //cout<<"(forceWithVirial->force_).size() "<<(forceWithVirial->force_).size()<<endl;
            //cout<<"Number of Atoms participating in do_force_lowlevel "<<fr->natoms_force<<endl;
            //cout<<"Number of Atoms participating in force and constraints "<<fr->natoms_force_constr<<endl;
            //cout<<"Number of Atoms in Array "<<mdatoms->nr<<endl;
...
static void do_nb_verlet(t_forcerec *fr,
                         ...
                         gmx_wallcycle_t wcycle)
{   
    //cout<<"static void do_nb_verlet is called!"<<endl;
    //cout<<"This file is sum_util.cpp, the function is do_nb_verlet, and *fr->print_force is: "<<fr->print_force<<endl;
...
static void do_nb_verlet_fep(nbnxn_pairlist_set_t *nbl_lists,
                             ...
                             gmx_wallcycle_t       wcycle)
{
    //cout<<"static void do_nb_verlet_fep is called!"<<endl;
    int              donb_flags;
...
{
    //cout<<"static real averageKineticENergyEstimate is called!"<<endl;
    real nrdfCoupled   = 0;
...
{
    //cout<<"static void checkPotentialEnergyValidity is called!"<<endl;
                     ...
                     gmx_bool          bNS)
{
    ...
    //cout<<"computeSpecialForces is called!"<<endl;
    //This is Called!
...
static inline void launchPmeGpuSpread(gmx_pme_t      *pmedata,
                                      ...
                                      gmx_wallcycle_t wcycle)
{
    //cout<<"static inline void launchPmeGpuSpread is called!"<<endl;
...
static void launchPmeGpuFftAndGather(gmx_pme_t        *pmedata,
                                     gmx_wallcycle_t   wcycle)
{
    //cout<<"static void launchPmeGpuFftAndGather is called!"<<endl;
...
static void alternatePmeNbGpuWaitReduce(nonbonded_verlet_t             *nbv,
                                        ...
                                        gmx_wallcycle_t                 wcycle)
{
    //cout<<"static void alternatePmeNbGpuWaitReduce is called!"<<endl;
                                           ...
                                           const gmx_int64_t         step)
{
     ...
     */
    //cout<<"static inline void launchGpuRollingPruning is called!"<<endl;
...
{
    //cout<<"This is sim_util.cpp and the function do_force_cutsVERLET is called!"<<endl;
    //cout<<"File *fplog data type: "<<typeid(fplog).name()<<endl;
    int                 cg1, i, j;
    double              mu[2*DIM];
    ...
    if (bStateChanged)
    {
        /*for(size_t i=0;i<sizeof(box);i++){
            for(size_t j=0;j<sizeof(box[i]);j++){
                cout<<"Before update i "<<i<<" j "<<j<<" box[i][j] "<<box[i][j]<<endl;
            }
        }*/        
        update_forcerec(fr, box);
	//cout<<"Is this called?"<<endl;
	//This is Called!
	//cout<<"sizeof(box): "<<sizeof(box)<<endl;
	/*for(size_t i=0;i<sizeof(box);i++){
	    for(size_t j=0;j<sizeof(box[i]);j++){
	    	cout<<"After update i "<<i<<" j "<<j<<" box[i][j] "<<box[i][j]<<endl;
	    }
	}*/
...
#if GMX_MPI
//send particle coordinates to the pme nodes is called
    if (!thisRankHasDuty(cr, DUTY_PME))
    {
        //cout<<"Send particle coordiantes to the pme nodes is called!"<<endl;
	...
	//This do_nb_verlet is NOT called!
    ...
    /* Temporary solution until all routines take PaddedRVecVector */
            //cout<<"rvec *f=as_rvec_array(force.data());"<<endl;
            //YES
	    ...
	    if (!bUseOrEmulGPU)
	    {   
		//cout<<"do_nb_verlet Called Once!"<<endl;
		//This do_nb_verlet is Called!
		//cout<<"enbvClearFYes data type: "<<typeid(enbvClearFYes).name()<<endl;
		//cout<<"enbvClearFYes is: "<<enbvClearFYes<<endl;
		//enbvClearFYes is 1
		do_nb_verlet(fr, ic, enerd, flags, eintLocal, enbvClearFYes,
			     step, nrnb, wcycle);
		//cout<<"sizeof((*nrnb).n) is: "<<sizeof((*nrnb).n)<<endl;
		/*for(size_t i=0;i<sizeof((*nrnb).n);i++){
			cout<<"i is: "<<i<<" (*nrnb).n[i] is: "<<(*nrnb).n[i]<<endl;
		}*/
		//cout<<"The number of atoms participating in do_force_lowlevel: "<<fr->natoms_force<<endl;
		//cout<<"The number of atoms participating in force and constraints: "<<fr->natoms_force_constr<<endl;
		//cout<<"The allocation size of vectors of size natoms_force: "<<fr->nalloc_force<<endl;
		//cout<<"The number of atoms in array: "<<mdatoms->nr<<endl;
		//cout<<"The number of perturbed atoms: "<<mdatoms->nPerturbed<<endl;
		//cout<<"The number of atoms for which the mass is perturbed: "<<mdatoms->nMassPerturbed<<endl;
		//cout<<"The number of atoms for which the charge is perturbed: "<<mdatoms->nChargePerturbed<<endl;
		//cout<<"The number of atoms for which the type is perturbed: "<<mdatoms->nTypePerturbed<<endl;
		
		//cout<<"sizeof(fr->posres_com): "<<sizeof(fr->posres_com)<<endl;
		//cout<<"sizeof(inputrec->posres_com): "<<sizeof(inputrec->posres_com)<<endl;
		//size of posrec_com is 12
		//cout<<"sizeof(fr->fshift): "<<sizeof(fr->fshift)<<endl;
		/*for(size_t i=0;i<sizeof(fr->fshift);i++){
			cout<<"i "<<i<<" *(fr->fshift)[i] "<<*(fr->fshift)[i]<<endl;
		}*/
		/*for(size_t i=0;i<sizeof(fr->posres_com);i++){
			cout<<"i: "<<i<<" (fr->posres_com)[i]: "<<(fr->posres_com)[i]<<" (inputrec->posres_com)[i]: "<<(inputrec->posres_com)[i]<<endl;
		}*/
		//cout<<"sizeof the pull vector, direction or position: "<<sizeof(*(inputrec->pull->coord->vec))<<endl;
		//cout<<"mass gradient weight for atoms: "<<sizeof(*(inputrec->pull_work->group->mdw))<<endl;
		//cout<<sizeof(*(inputrec->pull_work->group->mdw)[0])<<endl;
		//cout<<sizeof(inputrec->pull_work->group->mdw)<<endl;
		//cout<<sizeof(&inputrec->pull_work->group->mdw)<<endl;
		//cout<<sizeof(*(inputrec->pull_work->group->mdw))<<endl;
		//cout<<sizeof(inputrec->pull_work->group->mdw[0])<<endl;
		//cout<<sizeof((inputrec->pull_work->group->mdw)[0])<<endl;
		//cout<<sizeof(&inputrec->pull_work->group->mdw[0])<<endl;
		//cout<<sizeof((&inputrec->pull_work->group->mdw)[0])<<endl;
		//std::cout<<decltype((inputrec->pull_work->group->mdw)[0][0])<<endl;
		//cout<<inputrec->pull_work->group->x[0]<<endl;
		/*for(size_t i=0;i<sizeof(*(inputrec->pull_work->group->mdw));i++){
			for(size_t j=0;j<sizeof((inputrec->pull_work->group->mdw)[i]);j++){
				//cout<<"i: "<<i<<" j: "<<j<<" [i][j]: "<<*(inputrec->pull_work->group->mdw)[0][0]<<endl;
				//The cout does not work, but the nested for loop is correct.
				cout<<i<<" "<<j<<endl;
			}
		}*/
	    }
		...
		{   
		    //cout<<"Or is this called?????????????????????"<<endl;
		    //This is NOT called either!
		...
		if (DOMAINDECOMP(cr))
		{
		    //cout<<"do_nb_verlet Called Twice!"<<endl;
		    //This is Called!
			    do_nb_verlet(fr, ic, enerd, flags, eintNonlocal, enbvClearFNo,
					 step, nrnb, wcycle);
			}
			 ...
			 */
			double temp = wallcycle_stop(wcycle, ewcFORCE);
			//cout<<"Before Add: This file is sim_util.cpp, the function is do_force_cutsVERLET, and wallcycle_stop(wcycle, ewcFORCE) is: "<<temp<<endl;
			wallcycle_start(wcycle, ewcNB_XF_BUF_OPS);
			wallcycle_sub_start(wcycle, ewcsNB_F_BUF_OPS);
			nbnxn_atomdata_add_nbat_f_to_f(nbv->nbs, eatAll, nbv->nbat, f);
			wallcycle_sub_stop(wcycle, ewcsNB_F_BUF_OPS);
			temp=wallcycle_stop(wcycle, ewcNB_XF_BUF_OPS);
			//cout<<"After Add: This file is sim_util.cpp, the function is do_force_cutsVERLET, and wallcycle_stop(wcycle, ewcNB_XF_BUF_OPS) is: "<<temp<<endl;
		    ...
		    /* Compute the bonded and non-bonded energies and optionally forces */
                    //cout<<"do_force_lowlevel called"<<endl;
                    //Yes called
    ...
    //cout<<"in do_force_cutsVERLET right after do_force_lowlevel is called"<<endl;
    //cout<<(forceWithVirial.force_)[0][0]<<" "<<(forceWithVirial.force_)[0][1]<<" "<<(forceWithVirial.force_)[0][2]<<endl;
    ...
    if (alternateGpuWait)
    {
        //cout<<"alternatePmeNbGpuWaitReduce is called"<<endl;
        //NOT called
    ...
    if (!alternateGpuWait && useGpuPme)
    {
        ...
        //cout<<"pme_gpu_reduce_outputs is called"<<endl;
        //NOT called
    ...
    if (PAR(cr) && !thisRankHasDuty(cr, DUTY_PME))
    {
         ...
         * forces, virial and energy from the PME nodes here.
         */
        //cout<<"pme_receive_force_ener is called"<<endl;
        //Called
        pme_receive_force_ener(cr, &forceWithVirial, enerd, wcycle);
    }
    //cout<<"in do_force_cutsVERLET right before post_process is called"<<endl;
    //cout<<(forceWithVirial.force_)[0][0]<<" "<<(forceWithVirial.force_)[0][1]<<" "<<(forceWithVirial.force_)[0][2]<<endl;
    if (bDoForces)
    {
        //cout<<"Is this called?????????????????"<<endl;
...
static void do_force_cutsGROUP(FILE *fplog, t_commrec *cr,
                               ...
                               DdCloseBalanceRegionAfterForceComputation ddCloseBalanceRegion)
{
    //cout<<"static void do_force_cutsGROUP is called!"<<endl;
    int        cg0, cg1, i, j;
    double     mu[2*DIM];
    gmx_bool   bStateChanged, bNS, bFillGrid, bCalcCGCM;
    gmx_bool   bDoForces;
...
void do_force(FILE *fplog, t_commrec *cr,
              ...
              DdCloseBalanceRegionAfterForceComputation ddCloseBalanceRegion)
{
    //cout<<"void do_force is called!"<<endl;
...
{
    //cout<<"void do_constrain_first is called!"<<endl;
    int             i, m, start, end;
    gmx_int64_t     step;
    real            dt = ir->delta_t;
    real            dvdl_dum;
...
{
    //cout<<"static void integrate_table is called!"<<endl;
...
{
    //cout<<"void calc_enervirdiff is called!"<<endl;
...
{
    //cout<<"void calc_dispcorr is called!"<<endl;
...
static void low_do_pbc_mtop(FILE *fplog, int ePBC, const matrix box,
                            ...
                            gmx_bool bFirst)
{
    //cout<<"static void low_do_pbc_mtop is called!"<<endl;
    t_graph        *graph;
    int             mb, as, mol;
    gmx_molblock_t *molb;
...
void do_pbc_first_mtop(FILE *fplog, int ePBC, const matrix box,
                       const gmx_mtop_t *mtop, rvec x[])
{
    //cout<<"void do_pbc_first_mtop is called!"<<endl;
...
void finish_run(FILE *fplog, const gmx::MDLogger &mdlog, t_commrec *cr,
                ...
                gmx_bool bWriteStat)
{
    //cout<<"void finish_run is called!"<<endl;
...
extern void initialize_lambdas(FILE *fplog, t_inputrec *ir, int *fep_state, gmx::ArrayRef<real> lambda, double *lam0)
{
    ...
    //cout<<"external void initialize_lambdas is called!"<<endl;
...
void init_md(FILE *fplog,
             ...
             gmx_wallcycle_t wcycle)
{
    //cout<<"void init_md is called!"<<endl;
    int  i;
