                       ...
                       const rvec * gmx_restrict f)
{
    ...
    {
        //cout<<"lambdaGroup "<<lambdaGroup<<endl;
        //cout<<"tcstat[0].lambda "<<tcstat[0].lambda<<endl;
        //NOT called
    ...
    for (int a = start; a < nrend; a++)
    {
        ...
        //cout<<"nrend "<<nrend<<endl;
        if (numTempScaleValues == NumTempScaleValues::multiple)
        {
            //cout<<"lambdaGroup "<<lambdaGroup<<endl;
            //cout<<"cTC["<<a<<"] "<<cTC[a]<<endl;
            //cout<<"tcstat["<<cTC[a]<<"].lambda "<<tcstat[cTC[a]].lambda<<endl;
            //some numbers
                         ...
                         const matrix                M)
{
    ...
    {
        //cout<<"line 554 is called"<<endl;
        //NOT called
        matrix stepM;
        if (!doParrinelloRahman)
        {
            //cout<<"line 558 is called"<<endl;
            //NOT called
    ...
    {    
        ...
        if (doParrinelloRahman)
        {
            ...
            for (int d = 0; d < DIM; d++)
            {
                //cout<<"diagM["<<d<<"] "<<diagM[d]<<endl;
                //cout<<"M["<<d<<"]["<<d<<"] "<<M[d][d]<<endl;
                //A Bunch of Numbers
                diagM[d] = M[d][d];
            }
            ...
            {
                //cout<<"line 641 is called"<<endl;
                //Called
                //A bunch of numbers
                //xprime is indeed getting updated, but it doesn't look like x got copied n pasted to xprime. Numbers don't really match. 
                //cout<<"Before updateMDLeapfrogSimple called "<<endl;
                //cout<<"x[0][0] "<<x[0][0]<<" x[0][1] "<<x[0][1]<<" x[0][2] "<<x[0][2]<<endl;
                //cout<<"xprime[0][0] "<<xprime[0][0]<<" xprime[0][1] "<<xprime[0][1]<<" xprime[0][2] "<<xprime[0][2]<<endl;
                //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl;
                ...
                <NumTempScaleValues::multiple,
                 ApplyParrinelloRahmanVScaling::diagonal>
                    (start, nrend, dt, dtPressureCouple,
                    invMassPerDim, tcstat, cTC, diagM, x, xprime, v, f);
                //cout<<"After updateMDLeapfrogSimple called "<<endl;
                //cout<<"x[0][0] "<<x[0][0]<<" x[0][1] "<<x[0][1]<<" x[0][2] "<<x[0][2]<<endl;
                //cout<<"xprime[0][0] "<<xprime[0][0]<<" xprime[0][1] "<<xprime[0][1]<<" xprime[0][2] "<<xprime[0][2]<<endl;
                //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl;
            }
        ...
        {
            //cout<<"line 651 is called"<<endl;
            //Called
            if (haveSingleTempScaleValue)
            {
                //cout<<"line 654 is called"<<endl;
                //Called
                ...
                {
                    //cout<<"line 664 is called"<<endl;
                    //Called
                    //cout<<"Before updateMDLeapfrogSimpleSimd is called"<<endl;
                    //cout<<"x[0][0] "<<x[0][0]<<" x[0][1] "<<x[0][1]<<" x[0][2] "<<x[0][2]<<endl;
                    //cout<<"xprime[0][0] "<<xprime[0][0]<<" xprime[0][1] "<<xprime[0][1]<<" xprime[0][2] "<<xprime[0][2]<<endl;
                    //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl; 
                    updateMDLeapfrogSimpleSimd
                        (start, nrend, dt, md->invmass, tcstat, x, xprime, v, f);
                    //cout<<"After updateMDLeapfrogSimpleSimd is called"<<endl;
                    //cout<<"x[0][0] "<<x[0][0]<<" x[0][1] "<<x[0][1]<<" x[0][2] "<<x[0][2]<<endl;
                    //cout<<"xprime[0][0] "<<xprime[0][0]<<" xprime[0][1] "<<xprime[0][1]<<" xprime[0][2] "<<xprime[0][2]<<endl;
                    //cout<<"f[0][0] "<<f[0][0]<<" f[0][1] "<<f[0][1]<<" f[0][2] "<<f[0][2]<<endl;
                }
...
{
    //cout<<"do_update_vv_vel is called"<<endl;
    //NOT called
    int    gf = 0, ga = 0;
    int    n, d;
    real   g, mv1, mv2;
...
{
#ifdef DEBUG
    //cout<<"dump_it all is called "<<endl;
    //cout<<"natoms "<<natoms<<endl;
    //NOT called. Even simulation is finished
    if (fp)
    { 
        //cout<<"dump_it_all first if is called"<<endl;
        //NOT called
        fprintf(fp, "%s\n", title);
        ...
        if (f != NULL)
        {
            //cout<<"dump_it_all second if is called"<<endl;
            //NOT called
...
{
    int ccount=0;
    std::vector <std::vector<real>> carbon_coords;
    std::vector <std::vector<real>> carbon_forces;
    std::vector <int> carbon_index;
    std::vector <std::vector<real>> benzene_coords;
    std::vector <std::vector<real>> benzene_forces;
    std::vector <std::vector<int>> benzene_index;
...
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
    {
        try
        {
            //cout<<"in update.cpp function update_coords try is called"<<endl;
            //This is called!
            int start_th, end_th;
            getThreadAtomRange(nth, th, homenr, &start_th, &end_th);
            ...
            rvec *f_rvec  = as_rvec_array(f.data());
            //cout<<"In Update.cpp function Update_coords Before called"<<endl;
            //cout<<"x_rvec[0][0] "<<x_rvec[0][0]<<" x_rvec[0][1] "<<x_rvec[0][1]<<" x_rvec[0][2] "<<x_rvec[0][2]<<endl;
            //cout<<"xp_rvec[0][0] "<<xp_rvec[0][0]<<" xp_rvec[0][1] "<<xp_rvec[0][1]<<" xp_rvec[0][2] "<<xp_rvec[0][2]<<endl;
            //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
            switch (inputrec->eI)
            {
                case (eiMD):
                    //cout<<"do_update_md is called"<<endl;
                    //Called            
                    //cout<<"Before do_update_md is called "<<endl;
                    //cout<<"x_rvec[0][0] "<<x_rvec[0][0]<<" x_rvec[0][1] "<<x_rvec[0][1]<<" x_rvec[0][2] "<<x_rvec[0][2]<<endl;
                    //cout<<"xp_rvec[0][0] "<<xp_rvec[0][0]<<" xp_rvec[0][1] "<<xp_rvec[0][1]<<" xp_rvec[0][2] "<<xp_rvec[0][2]<<endl;
                    //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
                    ccount=0;
		    ...
		    carbon_forces.clear();
                    carbon_index.clear();
                    benzene_coords.clear();
                    benzene_forces.clear();
                    benzene_index.clear();
                    for(int i=start_th;i<end_th;i++){
			if(md->massT[i]>12.01&&md->massT[i]-12.01<0.002){
				carbon_coords.push_back(std::vector<real>());
				carbon_forces.push_back(std::vector<real>());
                                carbon_index.push_back(i);
				for(int j=0;j<3;j++){
					carbon_coords[ccount].push_back(x_rvec[i][j]);
					carbon_forces[ccount].push_back(f_rvec[i][j]);
                                        f_rvec[i][j]=f_rvec[i][j]+0;
				}
				ccount++;
			}
		    }
                    ccount=0;
		    for(int a=0;a<carbon_coords.size();a++){
			for(int b=a+1;b<carbon_coords.size();b++){
                            real temp_ab=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[b][0]),2)+pow((carbon_coords[a][1]-carbon_coords[b][1]),2)+pow((carbon_coords[a][2]-carbon_coords[b][2]),2));
                            if(temp_ab>3){
                            	continue;
                            }
			    for(int c=b+1;c<carbon_coords.size();c++){
                            	temp_ab=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[b][0]),2)+pow((carbon_coords[a][1]-carbon_coords[b][1]),2)+pow((carbon_coords[a][2]-carbon_coords[b][2]),2));
                                real temp_ac=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[c][0]),2)+pow((carbon_coords[a][1]-carbon_coords[c][1]),2)+pow((carbon_coords[a][2]-carbon_coords[c][2]),2));
                                real temp_bc=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[c][0]),2)+pow((carbon_coords[b][1]-carbon_coords[c][1]),2)+pow((carbon_coords[b][2]-carbon_coords[c][2]),2));
                                if(temp_ab>3 || temp_ac>3 || temp_bc>3){
				    continue;
                                }
			    	for(int d=c+1;d<carbon_coords.size();d++){
				    temp_ab=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[b][0]),2)+pow((carbon_coords[a][1]-carbon_coords[b][1]),2)+pow((carbon_coords[a][2]-carbon_coords[b][2]),2));
                                    temp_ac=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[c][0]),2)+pow((carbon_coords[a][1]-carbon_coords[c][1]),2)+pow((carbon_coords[a][2]-carbon_coords[c][2]),2));
                                    real temp_ad=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[d][0]),2)+pow((carbon_coords[a][1]-carbon_coords[d][1]),2)+pow((carbon_coords[a][2]-carbon_coords[d][2]),2));
                                    temp_bc=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[c][0]),2)+pow((carbon_coords[b][1]-carbon_coords[c][1]),2)+pow((carbon_coords[b][2]-carbon_coords[c][2]),2));
                                    real temp_bd=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[d][0]),2)+pow((carbon_coords[b][1]-carbon_coords[d][1]),2)+pow((carbon_coords[b][2]-carbon_coords[d][2]),2));
                                    real temp_cd=10*sqrt(pow((carbon_coords[c][0]-carbon_coords[d][0]),2)+pow((carbon_coords[c][1]-carbon_coords[d][1]),2)+pow((carbon_coords[c][2]-carbon_coords[d][2]),2));
                                    if(temp_ab>3||temp_ac>3||temp_ad>3||temp_bc>3||temp_bd>3||temp_cd>3){
                                        continue;
                                    }
				    for(int e=d+1;e<carbon_coords.size();e++){
                                        temp_ab=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[b][0]),2)+pow((carbon_coords[a][1]-carbon_coords[b][1]),2)+pow((carbon_coords[a][2]-carbon_coords[b][2]),2));
                                        temp_ac=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[c][0]),2)+pow((carbon_coords[a][1]-carbon_coords[c][1]),2)+pow((carbon_coords[a][2]-carbon_coords[c][2]),2));
                                        temp_ad=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[d][0]),2)+pow((carbon_coords[a][1]-carbon_coords[d][1]),2)+pow((carbon_coords[a][2]-carbon_coords[d][2]),2));
                                        real temp_ae=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[e][0]),2)+pow((carbon_coords[a][1]-carbon_coords[e][1]),2)+pow((carbon_coords[a][2]-carbon_coords[e][2]),2));
                                        temp_bc=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[c][0]),2)+pow((carbon_coords[b][1]-carbon_coords[c][1]),2)+pow((carbon_coords[b][2]-carbon_coords[c][2]),2));
                                        temp_bd=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[d][0]),2)+pow((carbon_coords[b][1]-carbon_coords[d][1]),2)+pow((carbon_coords[b][2]-carbon_coords[d][2]),2));
                                        real temp_be=10*sqrt(pow((carbon_coords[b][0]-carbon_coords[e][0]),2)+pow((carbon_coords[b][1]-carbon_coords[e][1]),2)+pow((carbon_coords[b][2]-carbon_coords[e][2]),2));
                                        temp_cd=10*sqrt(pow((carbon_coords[c][0]-carbon_coords[d][0]),2)+pow((carbon_coords[c][1]-carbon_coords[d][1]),2)+pow((carbon_coords[c][2]-carbon_coords[d][2]),2));
                                        real temp_ce=10*sqrt(pow((carbon_coords[c][0]-carbon_coords[e][0]),2)+pow((carbon_coords[c][1]-carbon_coords[e][1]),2)+pow((carbon_coords[c][2]-carbon_coords[e][2]),2));
                                        real temp_de=10*sqrt(pow((carbon_coords[d][0]-carbon_coords[e][0]),2)+pow((carbon_coords[d][1]-carbon_coords[e][1]),2)+pow((carbon_coords[d][2]-carbon_coords[e][2]),2));
                                        if(temp_ab>3||temp_ac>3||temp_ad>3||temp_ae>3||temp_bc>3||temp_bd>3||temp_be>3||temp_cd>3||temp_ce>3||temp_de>3){
                                            continue;
                                        }
					for(int f=e+1;f<carbon_coords.size();f++){
					    real center_x=(carbon_coords[a][0]+carbon_coords[b][0]+carbon_coords[c][0]+carbon_coords[d][0]+carbon_coords[e][0]+carbon_coords[f][0])/6;
					    real center_y=(carbon_coords[a][1]+carbon_coords[b][1]+carbon_coords[c][1]+carbon_coords[d][1]+carbon_coords[e][1]+carbon_coords[f][1])/6;
					    real center_z=(carbon_coords[a][2]+carbon_coords[b][2]+carbon_coords[c][2]+carbon_coords[d][2]+carbon_coords[e][2]+carbon_coords[f][2])/6;
                                            real dist_a=10*sqrt(pow((carbon_coords[a][0]-center_x),2)+pow((carbon_coords[a][1]-center_y),2)+pow((carbon_coords[a][2]-center_z),2));
                                            real dist_b=10*sqrt(pow((carbon_coords[b][0]-center_x),2)+pow((carbon_coords[b][1]-center_y),2)+pow((carbon_coords[b][2]-center_z),2));
                                            real dist_c=10*sqrt(pow((carbon_coords[c][0]-center_x),2)+pow((carbon_coords[c][1]-center_y),2)+pow((carbon_coords[c][2]-center_z),2));
                                            real dist_d=10*sqrt(pow((carbon_coords[d][0]-center_x),2)+pow((carbon_coords[d][1]-center_y),2)+pow((carbon_coords[d][2]-center_z),2));
                                            real dist_e=10*sqrt(pow((carbon_coords[e][0]-center_x),2)+pow((carbon_coords[e][1]-center_y),2)+pow((carbon_coords[e][2]-center_z),2));
					    real dist_f=10*sqrt(pow((carbon_coords[f][0]-center_x),2)+pow((carbon_coords[f][1]-center_y),2)+pow((carbon_coords[f][2]-center_z),2));
                                            //cout<<"a"<<"    "<<a<<"    "<<carbon_coords[a][0]<<"    "<<carbon_coords[a][1]<<"    "<<carbon_coords[a][2]<<endl;
                                            //cout<<"b"<<"    "<<b<<"    "<<carbon_coords[b][0]<<"    "<<carbon_coords[b][1]<<"    "<<carbon_coords[b][2]<<endl;
					    //cout<<"c"<<"    "<<c<<"    "<<carbon_coords[c][0]<<"    "<<carbon_coords[c][1]<<"    "<<carbon_coords[c][2]<<endl;
					    //cout<<"d"<<"    "<<d<<"    "<<carbon_coords[d][0]<<"    "<<carbon_coords[d][1]<<"    "<<carbon_coords[d][2]<<endl;
					    //cout<<"e"<<"    "<<e<<"    "<<carbon_coords[e][0]<<"    "<<carbon_coords[e][1]<<"    "<<carbon_coords[e][2]<<endl;
					    //cout<<"f"<<"    "<<f<<"    "<<carbon_coords[f][0]<<"    "<<carbon_coords[f][1]<<"    "<<carbon_coords[f][2]<<endl;
                                            //cout<<"center"<<"    "<<center_x<<"    "<<center_y<<"    "<<center_z<<endl;
                                            //cout<<"dist_a"<<"    "<<dist_a<<endl;
                                            //cout<<"dist_b"<<"    "<<dist_b<<endl;
                                            //cout<<"dist_c"<<"    "<<dist_c<<endl;
                                            //cout<<"dist_d"<<"    "<<dist_d<<endl;
                                            //cout<<"dist_e"<<"    "<<dist_e<<endl;
                                            //cout<<"dist_f"<<"    "<<dist_f<<endl;
                                            //cout<<dist_a<<"    "<<dist_b<<"    "<<dist_c<<"    "<<dist_d<<"    "<<dist_e<<"    "<<dist_f<<endl;
                                            if(dist_a>1.35&&dist_a<1.46){
						if(dist_b>1.35&&dist_b<1.46){
						    if(dist_c>1.35&&dist_c<1.46){
							if(dist_d>1.35&&dist_d<1.46){
							    if(dist_e>1.35&&dist_d<1.46){
								if(dist_f>1.35&&dist_f<1.46){
                                                                    //cout<<"####################################################################"<<endl;
								    //cout<<a<<"    "<<carbon_index[a]<<"    "<<carbon_coords[a][0]<<"    "<<carbon_coords[a][1]<<"    "<<carbon_coords[a][2]<<endl;
                                                		    //cout<<b<<"    "<<carbon_index[b]<<"    "<<carbon_coords[b][0]<<"    "<<carbon_coords[b][1]<<"    "<<carbon_coords[b][2]<<endl;
                                                		    //cout<<c<<"    "<<carbon_index[c]<<"    "<<carbon_coords[c][0]<<"    "<<carbon_coords[c][1]<<"    "<<carbon_coords[c][2]<<endl;
                                                		    //cout<<d<<"    "<<carbon_index[d]<<"    "<<carbon_coords[d][0]<<"    "<<carbon_coords[d][1]<<"    "<<carbon_coords[d][2]<<endl;
                                                		    //cout<<e<<"    "<<carbon_index[e]<<"    "<<carbon_coords[e][0]<<"    "<<carbon_coords[e][1]<<"    "<<carbon_coords[e][2]<<endl;
                                                		    //cout<<f<<"    "<<carbon_index[f]<<"    "<<carbon_coords[f][0]<<"    "<<carbon_coords[f][1]<<"    "<<carbon_coords[f][2]<<endl;
                                                		    //real dist_ab=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[b][0]),2)+pow((carbon_coords[a][1]-carbon_coords[b][1]),2)+pow((carbon_coords[a][2]-carbon_coords[b][2]),2));
								    //real dist_ac=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[c][0]),2)+pow((carbon_coords[a][1]-carbon_coords[c][1]),2)+pow((carbon_coords[a][2]-carbon_coords[c][2]),2));
                                                                    //real dist_ad=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[d][0]),2)+pow((carbon_coords[a][1]-carbon_coords[d][1]),2)+pow((carbon_coords[a][2]-carbon_coords[d][2]),2));
                                                                    //real dist_ae=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[e][0]),2)+pow((carbon_coords[a][1]-carbon_coords[e][1]),2)+pow((carbon_coords[a][2]-carbon_coords[e][2]),2));
                                                                    //real dist_af=10*sqrt(pow((carbon_coords[a][0]-carbon_coords[f][0]),2)+pow((carbon_coords[a][1]-carbon_coords[f][1]),2)+pow((carbon_coords[a][2]-carbon_coords[f][2]),2));
                                                                    //cout<<"dist_ab  "<<dist_ab<<"  dist_ac  "<<dist_ac<<"  dist_ad  "<<dist_ad<<"  dist_ae  "<<dist_ae<<"  dist_af  "<<dist_af<<endl;
                                                                    benzene_coords.push_back(std::vector<real>());
                                                                    benzene_coords[ccount].push_back(carbon_coords[a][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[a][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[a][2]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[b][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[b][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[b][2]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[c][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[c][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[c][2]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[d][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[d][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[d][2]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[e][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[e][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[e][2]); 
                                                                    benzene_coords[ccount].push_back(carbon_coords[f][0]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[f][1]);
                                                                    benzene_coords[ccount].push_back(carbon_coords[f][2]);
                                                                    benzene_forces.push_back(std::vector<real>());
                                                                    benzene_forces[ccount].push_back(carbon_forces[a][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[a][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[a][2]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[b][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[b][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[b][2]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[c][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[c][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[c][2]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[d][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[d][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[d][2]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[e][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[e][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[e][2]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[f][0]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[f][1]);
                                                                    benzene_forces[ccount].push_back(carbon_forces[f][2]);
                                                                    benzene_index.push_back(std::vector<int>());
                                                                    benzene_index[ccount].push_back(carbon_index[a]);
                                                                    benzene_index[ccount].push_back(carbon_index[b]);
                                                                    benzene_index[ccount].push_back(carbon_index[c]);
                                                                    benzene_index[ccount].push_back(carbon_index[d]);
                                                                    benzene_index[ccount].push_back(carbon_index[e]);
                                                                    benzene_index[ccount].push_back(carbon_index[f]); 
                                                                    ccount++;
								}
							    }
							}
                    ...
                    //for(int i=0;i<benzene_coords.size();i++){
			//cout<<"Ring Coordinates  "<<benzene_coords[i][0]<<"  "<<benzene_coords[i][1]<<"  "<<benzene_coords[i][2]<<"  "<<benzene_coords[i][3]<<"  "<<benzene_coords[i][4]<<"  "<<benzene_coords[i][5]<<endl;
                        //cout<<"Ring Coordinates  "<<benzene_coords[i][6]<<"  "<<benzene_coords[i][7]<<"  "<<benzene_coords[i][8]<<"  "<<benzene_coords[i][9]<<"  "<<benzene_coords[i][10]<<"  "<<benzene_coords[i][11]<<endl;
                        //cout<<"Ring Coordinates  "<<benzene_coords[i][12]<<"  "<<benzene_coords[i][13]<<"  "<<benzene_coords[i][14]<<"  "<<benzene_coords[i][15]<<"  "<<benzene_coords[i][16]<<"  "<<benzene_coords[i][17]<<endl;
                        //cout<<"Ring Forces  "<<benzene_forces[i][0]<<"  "<<benzene_forces[i][1]<<"  "<<benzene_forces[i][2]<<"  "<<benzene_forces[i][3]<<"  "<<benzene_forces[i][4]<<"  "<<benzene_forces[i][5]<<endl;
                        //cout<<"Ring Forces  "<<benzene_forces[i][6]<<"  "<<benzene_forces[i][7]<<"  "<<benzene_forces[i][8]<<"  "<<benzene_forces[i][9]<<"  "<<benzene_forces[i][10]<<"  "<<benzene_forces[i][11]<<endl;
                        //cout<<"Ring Forces  "<<benzene_forces[i][12]<<"  "<<benzene_forces[i][13]<<"  "<<benzene_forces[i][14]<<"  "<<benzene_forces[i][15]<<"  "<<benzene_forces[i][16]<<"  "<<benzene_forces[i][17]<<endl;
                        //cout<<"Ring Indices "<<benzene_index[i][0]<<"  "<<benzene_index[i][1]<<"  "<<benzene_index[i][2]<<"  "<<benzene_index[i][3]<<"  "<<benzene_index[i][4]<<"  "<<benzene_index[i][5]<<endl;
                        //cout<<"f_rvec  "<<f_rvec[benzene_index[i][0]][0]<<"  "<<f_rvec[benzene_index[i][0]][1]<<"  "<<f_rvec[benzene_index[i][0]][2]<<endl;
                        //cout<<"x_rvec  "<<x_rvec[benzene_index[i][0]][0]<<"  "<<x_rvec[benzene_index[i][0]][1]<<"  "<<x_rvec[benzene_index[i][0]][2]<<endl;
                        //cout<<"###################################################################################"<<endl;
		    //}
                    for(int i=0;i<benzene_coords.size();i++){
			for(int j=i+1;j<benzene_coords.size();j++){
				real x1_center=(benzene_coords[i][0]+benzene_coords[i][3]+benzene_coords[i][6]+benzene_coords[i][9]+benzene_coords[i][12]+benzene_coords[i][15])/6;
				real y1_center=(benzene_coords[i][1]+benzene_coords[i][4]+benzene_coords[i][7]+benzene_coords[i][10]+benzene_coords[i][13]+benzene_coords[i][16])/6;
				real z1_center=(benzene_coords[i][2]+benzene_coords[i][5]+benzene_coords[i][8]+benzene_coords[i][11]+benzene_coords[i][14]+benzene_coords[i][17])/6;
                                real x2_center=(benzene_coords[j][0]+benzene_coords[j][3]+benzene_coords[j][6]+benzene_coords[j][9]+benzene_coords[j][12]+benzene_coords[j][15])/6;
                                real y2_center=(benzene_coords[j][1]+benzene_coords[j][4]+benzene_coords[j][7]+benzene_coords[j][10]+benzene_coords[j][13]+benzene_coords[j][16])/6;
                                real z2_center=(benzene_coords[j][2]+benzene_coords[j][5]+benzene_coords[j][8]+benzene_coords[j][11]+benzene_coords[j][14]+benzene_coords[j][17])/6;
                                real R=10*sqrt(pow((x1_center-x2_center),2)+pow((y1_center-y2_center),2)+pow((z1_center-z2_center),2));
                                real RV=10*std::abs(z1_center-z2_center);
                                real RH=10*sqrt(pow((x1_center-x2_center),2)+pow((y1_center-y2_center),2));
                                cout<<"Top Ring Coordinates  "<<benzene_coords[i][0]<<"  "<<benzene_coords[i][1]<<"  "<<benzene_coords[i][2]<<"  "<<benzene_coords[i][3]<<"  "<<benzene_coords[i][4]<<"  "<<benzene_coords[i][5]<<endl;
                                cout<<"Top Ring Coordinates  "<<benzene_coords[i][6]<<"  "<<benzene_coords[i][7]<<"  "<<benzene_coords[i][8]<<"  "<<benzene_coords[i][9]<<"  "<<benzene_coords[i][10]<<"  "<<benzene_coords[i][11]<<endl;
                                cout<<"Top Ring Coordinates  "<<benzene_coords[i][12]<<"  "<<benzene_coords[i][13]<<"  "<<benzene_coords[i][14]<<"  "<<benzene_coords[i][15]<<"  "<<benzene_coords[i][16]<<"  "<<benzene_coords[i][17]<<endl;
                                cout<<"Btm Ring Coordinates  "<<benzene_coords[j][0]<<"  "<<benzene_coords[j][1]<<"  "<<benzene_coords[j][2]<<"  "<<benzene_coords[j][3]<<"  "<<benzene_coords[j][4]<<"  "<<benzene_coords[j][5]<<endl;
                                cout<<"Btm Ring Coordinates  "<<benzene_coords[j][6]<<"  "<<benzene_coords[j][7]<<"  "<<benzene_coords[j][8]<<"  "<<benzene_coords[j][9]<<"  "<<benzene_coords[j][10]<<"  "<<benzene_coords[j][11]<<endl;
                                cout<<"Btm Ring Coordinates  "<<benzene_coords[j][12]<<"  "<<benzene_coords[j][13]<<"  "<<benzene_coords[j][14]<<"  "<<benzene_coords[j][15]<<"  "<<benzene_coords[j][16]<<"  "<<benzene_coords[j][17]<<endl;
                                cout<<"Top Ring Forces  "<<benzene_forces[i][0]<<"  "<<benzene_forces[i][1]<<"  "<<benzene_forces[i][2]<<"  "<<benzene_forces[i][3]<<"  "<<benzene_forces[i][4]<<"  "<<benzene_forces[i][5]<<endl;
                                cout<<"Top Ring Forces  "<<benzene_forces[i][6]<<"  "<<benzene_forces[i][7]<<"  "<<benzene_forces[i][8]<<"  "<<benzene_forces[i][9]<<"  "<<benzene_forces[i][10]<<"  "<<benzene_forces[i][11]<<endl;
                                cout<<"Top Ring Forces  "<<benzene_forces[i][12]<<"  "<<benzene_forces[i][13]<<"  "<<benzene_forces[i][14]<<"  "<<benzene_forces[i][15]<<"  "<<benzene_forces[i][16]<<"  "<<benzene_forces[i][17]<<endl;
                                cout<<"Btm Ring Forces  "<<benzene_forces[j][0]<<"  "<<benzene_forces[j][1]<<"  "<<benzene_forces[j][2]<<"  "<<benzene_forces[j][3]<<"  "<<benzene_forces[j][4]<<"  "<<benzene_forces[j][5]<<endl;
                                cout<<"Btm Ring Forces  "<<benzene_forces[j][6]<<"  "<<benzene_forces[j][7]<<"  "<<benzene_forces[j][8]<<"  "<<benzene_forces[j][9]<<"  "<<benzene_forces[j][10]<<"  "<<benzene_forces[j][11]<<endl;
                                cout<<"Btm Ring Forces  "<<benzene_forces[j][12]<<"  "<<benzene_forces[j][13]<<"  "<<benzene_forces[j][14]<<"  "<<benzene_forces[j][15]<<"  "<<benzene_forces[j][16]<<"  "<<benzene_forces[j][17]<<endl;
                                cout<<"Top rvec Forces  "<<f_rvec[benzene_index[i][0]][0]<<"  "<<f_rvec[benzene_index[i][0]][1]<<"  "<<f_rvec[benzene_index[i][0]][2]<<"  "<<f_rvec[benzene_index[i][1]][0]<<"  "<<f_rvec[benzene_index[i][1]][1]<<"  "<<f_rvec[benzene_index[i][1]][2]<<endl;
                                cout<<"Top rvec Forces  "<<f_rvec[benzene_index[i][2]][0]<<"  "<<f_rvec[benzene_index[i][2]][1]<<"  "<<f_rvec[benzene_index[i][2]][2]<<"  "<<f_rvec[benzene_index[i][3]][0]<<"  "<<f_rvec[benzene_index[i][3]][1]<<"  "<<f_rvec[benzene_index[i][3]][2]<<endl;
                                cout<<"Top rvec Forces  "<<f_rvec[benzene_index[i][4]][0]<<"  "<<f_rvec[benzene_index[i][4]][1]<<"  "<<f_rvec[benzene_index[i][4]][2]<<"  "<<f_rvec[benzene_index[i][5]][0]<<"  "<<f_rvec[benzene_index[i][5]][1]<<"  "<<f_rvec[benzene_index[i][5]][2]<<endl; 
                                cout<<"Btm rvec Forces  "<<f_rvec[benzene_index[j][0]][0]<<"  "<<f_rvec[benzene_index[j][0]][1]<<"  "<<f_rvec[benzene_index[j][0]][2]<<"  "<<f_rvec[benzene_index[j][1]][0]<<"  "<<f_rvec[benzene_index[j][1]][1]<<"  "<<f_rvec[benzene_index[j][1]][2]<<endl;
                                cout<<"Btm rvec Forces  "<<f_rvec[benzene_index[j][2]][0]<<"  "<<f_rvec[benzene_index[j][2]][1]<<"  "<<f_rvec[benzene_index[j][2]][2]<<"  "<<f_rvec[benzene_index[j][3]][0]<<"  "<<f_rvec[benzene_index[j][3]][1]<<"  "<<f_rvec[benzene_index[j][3]][2]<<endl;
                                cout<<"Btm rvec Forces  "<<f_rvec[benzene_index[j][4]][0]<<"  "<<f_rvec[benzene_index[j][4]][1]<<"  "<<f_rvec[benzene_index[j][4]][2]<<"  "<<f_rvec[benzene_index[j][5]][0]<<"  "<<f_rvec[benzene_index[j][5]][1]<<"  "<<f_rvec[benzene_index[j][5]][2]<<endl;
                                cout<<"Top Ring Center  "<<x1_center<<"  "<<y1_center<<"  "<<z1_center<<endl;
                                cout<<"Btm Ring Center  "<<x2_center<<"  "<<y2_center<<"  "<<z2_center<<endl;
                                cout<<"R  "<<R<<endl;
                                cout<<"RH  "<<RH<<endl;
                                cout<<"RV  "<<RV<<endl;
                                real theta_1;
                                theta_1 = std::acos((benzene_coords[i][0]-x1_center)/sqrt(pow((benzene_coords[i][0]-x1_center),2)+pow((benzene_coords[i][1]-y1_center),2)+pow((benzene_coords[i][2]-z1_center),2)))*180/PI;
                                real theta_2;
                                theta_2 = std::acos((benzene_coords[j][0]-x2_center)/sqrt(pow((benzene_coords[j][0]-x2_center),2)+pow((benzene_coords[j][1]-y2_center),2)+pow((benzene_coords[j][2]-z2_center),2)))*180/PI;
                                real theta;
                                theta = std::abs(theta_1-theta_2);
                                cout<<"Theta  "<<theta<<endl;
                                real phi_1, phi_2, phi;
                                phi_1 = std::acos((benzene_coords[i][1]-y1_center)/(sqrt(pow((benzene_coords[i][1]-y1_center),2)+pow((benzene_coords[i][2]-z1_center),2))))*180/PI;
                                phi_2 = std::acos((benzene_coords[j][1]-y2_center)/(sqrt(pow((benzene_coords[j][1]-y2_center),2)+pow((benzene_coords[j][2]-z2_center),2))))*180/PI;
                                phi=std::abs(phi_1-phi_2);
                                cout<<"Phi  "<<phi<<endl;
                                cout<<"##########################################################################"<<endl;
			}
		    }
                    //double WTF;
                    //WTF=gmx::acos(0.5)*180/PI; 
                    //cout<<WTF<<endl;
                    //double jesus;
                    //jesus=gmx::abs(1.0-2.0);
                    //cout<<jesus<<endl; 
                    do_update_md(start_th, end_th, step, dt,
                                 inputrec, md, ekind, state->box,
                                 x_rvec, xp_rvec, v_rvec, f_rvec,
                                 state->nosehoover_vxi.data(), M);
                    //cout<<"After do_update_md is called "<<endl;
                    //cout<<"x_rvec[0][0] "<<x_rvec[0][0]<<" x_rvec[0][1] "<<x_rvec[0][1]<<" x_rvec[0][2] "<<x_rvec[0][2]<<endl;
                    //cout<<"xp_rvec[0][0] "<<xp_rvec[0][0]<<" xp_rvec[0][1] "<<xp_rvec[0][1]<<" xp_rvec[0][2] "<<xp_rvec[0][2]<<endl;
                    //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
                    //ccount=0;
                    /*for(int i=start_th;i<end_th;i++){
			//cout<<"xp_rvec["<<i<<"][0] "<<xp_rvec[i][0]<<" xp_rvec["<<i<<"][1] "<<xp_rvec[i][1]<<" xp_rvec["<<i<<"][2] "<<xp_rvec[i][2]<<endl;
                        //cout<<"md->massT["<<i<<"] "<<(md->massT)[i]<<endl;
                        //cout<<x_rvec[i][0]<<"    "<<x_rvec[i][1]<<"    "<<x_rvec[i][2]<<endl;
			cout<<"x_rvec["<<i<<"][0] "<<x_rvec[i][0]<<" x_rvec["<<i<<"][1] "<<x_rvec[i][1]<<" x_rvec["<<i<<"][2] "<<x_rvec[i][2]<<endl;
		        //if (md->massT[i]>12.01 && md->massT[i]-12.01<0.002){
				//cout<<"xp_rvec["<<i<<"][0] "<<xp_rvec[i][0]<<" xp_rvec["<<i<<"][1] "<<xp_rvec[i][1]<<" xp_rvec["<<i<<"][2] "<<xp_rvec[i][2]<<endl;
                                //cout<<"md->massT["<<i<<"] "<<(md->massT)[i]<<endl;
                                //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
                                //cout<<ccount<<" "<<xp_rvec[i][0]<<" "<<xp_rvec[i][1]<<" "<<xp_rvec[i][2]<<endl;
                                //ccount++;
                        //}
		    }*/
                    //Check if can cout xp_rvec with state->natoms
		    ...
		    //}
		    //Yes can
                    //float carbon_coords[ccount][3];
                    //float carbon_forces[ccount][3];
                    //ccount=0;
                    //carbon_coords[0]={1.1,2.2,3.3};
                    //carbon_coords.clear();
                    //carbon_forces.clear();
                    //for(int i=start_th;i<end_th;i++){
			//if(md->massT[i]>12.01&&md->massT[i]-12.01<0.002){
				//carbon_coords.push_back(std::vector<float>());
                                //carbon_forces.push_back(std::vector<float>());
				//for(int j=0;j<3;j++){
					//carbon_coords[ccount].push_back(xp_rvec[i][j]);
                                        //carbon_forces[ccount].push_back(f_rvec[i][j]);
				//}
				//ccount++;
			//}
		    //}
		    /*for(int i=0;i<carbon_coords.size();i++){
			cout<<"carbon_coords["<<i<<"][0] "<<carbon_coords[i][0]<<" carbon_coords["<<i<<"][1] "<<carbon_coords[i][1]<<" carbon_coords["<<i<<"]{2] "<<carbon_coords[i][2]<<endl;
                    	cout<<"carbon_forces["<<i<<"][0] "<<carbon_forces[i][0]<<" carbon_forces["<<i<<"][1] "<<carbon_forces[i][1]<<" carbon_forces["<<i<<"][2] "<<carbon_forces[i][2]<<endl;
		    }*/ 
                    break;
                case (eiSD1):
                    /* With constraints, the SD1 update is done in 2 parts */
                    //cout<<"do_update_sdl is called"<<endl;
                    //NOT called
                    do_update_sd1(upd->sd,
                                  start_th, end_th, dt,
                                  inputrec->opts.acc, inputrec->opts.nFreeze,
                    ...
                    break;
                case (eiBD):
                    //cout<<"do_update_bd is called"<<endl;
                    //NOT Called
            ...
            //cout<<"In Update.cpp function Update_coords After called"<<endl;
            //cout<<"x_rvec[0][0] "<<x_rvec[0][0]<<" x_rvec[0][1] "<<x_rvec[0][1]<<" x_rvec[0][2] "<<x_rvec[0][2]<<endl;
            //cout<<"xp_rvec[0][0] "<<xp_rvec[0][0]<<" xp_rvec[0][1] "<<xp_rvec[0][1]<<" xp_rvec[0][2] "<<xp_rvec[0][2]<<endl;
            //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
            //cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;
