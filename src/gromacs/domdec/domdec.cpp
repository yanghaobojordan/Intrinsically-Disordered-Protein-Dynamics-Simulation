            ...
            else if (!bScrew)
            {
                for (i = 0; i < ind->nsend[nzone]; i++)
                {
                    ...
                    for (j = at0; j < at1; j++)
                    {
                        ...
                        //cout<<"x["<<j<<"][0] "<<x[j][0]<<" x["<<j<<"][1] "<<x[j][1]<<" x["<<j<<"][2] "<<x[j][2]<<endl;
                        //cout<<"shift[0] "<<shift[0]<<" shift[1] "<<shift[1]<<" shift[2] "<<shift[2]<<endl;
                        //cout<<"buf["<<n<<"][0] "<<buf[n][0]<<" buf["<<n<<"][1] "<<buf[n][1]<<" buf["<<n<<"][2] "<<buf[n][2]<<endl;
                          ...
                          gmx_domdec_t *dd)
{
    ...
    for (icg = 0; icg < cgs->nr; icg++)
    {
        ...
        else
        {
            ...
            for (k = k0; (k < k1); k++)
            {
                //cout<<"Before Add cg_cm[0] "<<cg_cm[0]<<" cg_cm[1] "<<cg_cm[1]<<" cg_cm[2] "<<cg_cm[2]<<endl;
                //cout<<"Before Add pos["<<k<<"][0] "<<pos[k][0]<<" pos["<<k<<"][1] "<<pos[k][1]<<" pos["<<k<<"][2] "<<pos[k][2]<<endl;
                rvec_inc(cg_cm, pos[k]);
                //cout<<"line 3637"<<endl;
                //cout<<"After Add cg_cm[0] "<<cg_cm[0]<<" cg_cm[1] "<<cg_cm[1]<<" cg_cm[2] "<<cg_cm[2]<<endl;
                //cout<<"After Add pos["<<k<<"][0] "<<pos[k][0]<<" pos["<<k<<"][1] "<<pos[k][1]<<" pos["<<k<<"][2] "<<pos[k][2]<<endl;
            }
            for (d = 0; (d < DIM); d++)
            {
                cg_cm[d] *= inv_ncg;
            ...
            if (dd->nc[d] > 1)
            {
                ...
                else if (pos_d < cell_x0[d])
                {
                    ...
                    if (dd->ci[d] == 0)
                    {
                        ...
                        {
                            //cout<<"Before Add"<<endl;
                            //cout<<"state->x["<<k<<"][0] "<<state->x[k][0]<<" state->x["<<k<<"][1] "<<state->x[k][1]<<" state->x["<<k<<"][2] "<<state->x[k][2]<<endl;
                            //cout<<"state->box["<<d<<"][0] "<<state->box[d][0]<<" state->box["<<d<<"][1] "<<state->box[d][1]<<" state->box["<<d<<"][2] "<<state->box[d][2]<<endl;
