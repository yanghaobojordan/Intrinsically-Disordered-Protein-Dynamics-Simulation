                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    while (!bLastStep)
    {
        ...
        else
        {
             ...
             */
            //cout<<"programs/mdrun/md.cpp do_force is called!"<<endl;
            //This is the only place that do_force is Called
        ...
        /*  ############### START FIRST UPDATE HALF-STEP FOR VV METHODS############### */
        {
            ...
            //cout<<"update_coords is called in md.cpp"<<endl;
            //NOT called
        ...
        if (!bRerunMD || rerun_fr.bV || bForceUpdate)
        {
            ...
            if (EI_VV(ir->eI))
            {
                ...
                //cout<<"velocity half-step update is called"<<endl;
                //NOT called
                update_coords(fplog, step, ir, mdatoms, state, f, fcd,
            ...
            //cout<<"line 1608 update_coords is called"<<endl;
            //This one is Called
     
            //rvec *x_rvec  = as_rvec_array(state->x.data());
	    ...
	    //cout<<"in md.cpp Before update_coords is called "<<endl;
            ...
            //cout<<"v_rvec[0][0] "<<v_rvec[0][0]<<" v_rvec[0][1] "<<v_rvec[0][1]<<" v_rvec[0][2] "<<v_rvec[0][2]<<endl;
            //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;

            update_coords(fplog, step, ir, mdatoms, state, f, fcd,
                          ekind, M, upd, etrtPOSITION, cr, constr);
            //cout<<"in md.cpp After update_coords is called "<<endl;
            //cout<<"x_rvec[0][0] "<<x_rvec[0][0]<<" x_rvec[0][1] "<<x_rvec[0][1]<<" x_rvec[0][2] "<<x_rvec[0][2]<<endl;
            //cout<<"xp_rvec[0][0] "<<xp_rvec[0][0]<<" xp_rvec[0][1] "<<xp_rvec[0][1]<<" xp_rvec[0][2] "<<xp_rvec[0][2]<<endl;
            //cout<<"v_rvec[0][0] "<<v_rvec[0][0]<<" v_rvec[0][1] "<<v_rvec[0][1]<<" v_rvec[0][2] "<<v_rvec[0][2]<<endl;
            //cout<<"f_rvec[0][0] "<<f_rvec[0][0]<<" f_rvec[0][1] "<<f_rvec[0][1]<<" f_rvec[0][2] "<<f_rvec[0][2]<<endl;
            ...
            if (ir->eI == eiVVAK)
            {
                ...
                //cout<<"line 1634 update_coords is called"<<endl;
                //NOT called
                update_coords(fplog, step, ir, mdatoms, state, f, fcd,
                              ekind, M, upd, etrtPOSITION, cr, constr);
                wallcycle_stop(wcycle, ewcUPDATE);
